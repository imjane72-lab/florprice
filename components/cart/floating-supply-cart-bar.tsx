...
interface FloatingSupplyCartBarProps {
  selectedSupplies: Supply[];
  onRemove: (id: string) => void;
}

export function FloatingSupplyCartBar({
  selectedSupplies,
  onRemove,
}: FloatingSupplyCartBarProps) {
  const [isExpanded, setIsExpanded] = useState(false);

  if (selectedSupplies.length === 0) return null;

  const totalPrice = selectedSupplies.reduce((sum, s) => {
    const price = s.isSale && s.salePrice ? s.salePrice : s.price;
    return sum + price;
  }, 0);
